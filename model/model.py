...
#used mnist dataset from scikit learn
#* This model has 2 layer and has 10 neruron in each layer (excluding input layer)
#* activation func: ReLu and softmax
...
class recog:
    ...
    def back_prop(self):
        ...
        self.db2 = 1/self.m * np.sum(self.dZ2, 1)
        self.dZ1 = self.w2.T @(self.dZ2 * deriv_ReLu(self.Z1))
        self.dw1 = 1/self.m * self.dZ1.dot(self.X.T)
        self.db1 = 1/self.m * np.sum(self.dZ1, 1)

    def update_param(self):
        self.w1 = self.w1 - self.lr * self.dw1
        self.b1 = self.b1 - self.lr * np.reshape(self.db1, (10, 1))
        self.w2 = self.w2 - self.lr * self.dw2
        self.b2 = self.b2 - self.lr * np.reshape(self.db2, (10, 1))
    ...
    def gradient_decent(self):
        for i in range(self.epochs):
            ...
            self.update_param()
            if i % 50 == 0:
                accuracy_report(self.A2, self.y)         
                print(f"Epoch: {i} \n")
